...
**seed 1本**で、探索配分が PPL でどの一様配分より良いことを見た。report/04
（wikitext2 / c4 校正、seed 3本）では逆に、探索配分は最良の一様を上回らなかった。
ここは seed 0 の観測が他の seed でも出るかを見る。

1 seed は3段でできている。**済んだ段は飛ばす**ので、seed 0 は探索とベンチが
既にあり、採点だけが走る。

  1. 探索  ``cmoe search`` 幅2 / 3 / 4      → result_logs/slimpajama_w<W>_n16_seed<N>
  2. 採点  ``cmoe score``  一様 x=0..6      → result_logs/score_uniform_slimpajama_n16_seed<N>
  3. 測定  ``cmoe run --bench`` 10構成      → result_logs/bench_slimpajama_seed<N>

段2 がこの実験で新しいところである。**探索は校正データだけを見て配分を1本選ぶ**
のに、対照の一様配分を「評価指標を見て7本の最良」で選ぶと、対照の側にだけ
たまたまの上振れが乗る（report/04 で最良の一様が seed と指標ごとに変わったのが
その現れである）。段2 は一様7本を探索と同じ ``suffix_kl`` で採点するので、
その最良を取れば両側とも「校正だけで選んだ1本」になる。

dense の基準は測らない。report/04 のものを取り込む（dense は変換していない
モデルなので、校正にも seed にも配分にも依らない）。

  uv run python experiments/06_slimpajama_seeds/run.py --smoke   # 2層・8問
...
# （report/06 と同じ並び。並べ替えると「違う実験」と判定されて済んだ段が引き直る）
BASELINE = 'uniform3'
UNIFORMS = [BASELINE] + [f'uniform{x}' for x in (0, 1, 2, 4, 5, 6)]
# 段2 で採点する一様。x の小さい順に並べる（表として読むのはこちらの順）
SCORED_UNIFORMS = [f'uniform{x}' for x in range(7)]
...
def bench_stage(seed, root, plan, searched):
    ...
    argv += ['--router', ROUTER, '--seeds', str(seed), '--calib', CALIB,
             '--nsamples', str(plan['nsamples']), '--datasets', DATASETS,
...
def main(argv=None):
    ...
    plan = {
        ...
        # 構成数がそのまま時間になる
        'uniforms': [BASELINE, 'uniform4'] if args.smoke else UNIFORMS,
        'scored_uniforms': ['uniform3', 'uniform4'] if args.smoke
    ...
    if args.smoke:
        default_root = default_root / 'exp06_smoke'
    ...
    summarize(root / f'exp06_stages_seed{args.seed}.json', record)
