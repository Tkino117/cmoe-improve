"""06 の複数 seed をまとめる。GPU は使わない。

各 seed の ``bench_slimpajama_seed<N>/summary.json`` と ``bench/*.json``、それに
``score_uniform_slimpajama_n16_seed<N>/score.json`` を読み、構成ごとに seed 間の
平均と、対照との差を出す。生の対数尤度が残っているので、指標も対応のある比較も
ここで作り直せる（測り直しは要らない）。
...
1. ``uniform3`` … 固定。report/04・06 が使った基準で、選び方に評価指標が入らない
2. **校正で選んだ一様** … 一様7本を探索と同じ ``suffix_kl`` で採点した最良。
   探索も対照も「校正だけを見て選んだ1本」になるので、これが対等な比較である
3. 評価指標で最良の一様 … その指標を見てから7本の最良を取る。対照の側にだけ
...
from run import CALIB, NSAMPLES, ROOT, WIDTHS  # noqa: E402
...
def load_seed(root, seed, nsamples, widths=WIDTHS):
    ...
    stage = root / f'bench_{CALIB}_seed{seed}'
    ...
    score_path = (root / f'score_uniform_{CALIB}_n{nsamples}_seed{seed}'
                  / 'score.json')
    calib_choice = None
...
def render(table, out):
    seeds = table['seeds']
    labels = table['labels']
    lines = [f'# 06 slimpajama seeds まとめ（seed {", ".join(str(s) for s in seeds)}）',
             '', f'問題数: ' + ' / '.join(f'{task} {n}'
                                          for task, n in table['n_docs'].items()), '']

    lines += ['## 校正で選ばれた一様配分', '',
              '一様 x=0〜6 を探索と同じ `suffix_kl` で採点した結果。'
...
def main(argv=None):
    ...
    out = Path(args.out) if args.out else root / 'exp06_seeds'
