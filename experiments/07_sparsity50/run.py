...
[report/07](../../report/07_slimpajama-alloc-3seeds.md) は N=8 / A=6（スパース率
25%）で、探索配分が **PPL では3対照すべてに勝ち**、**ベンチでは検出できない**
ことを見た。ベンチの効果量が `acc` で 0.005 程度、区間の半幅が ±0.005 で、
seed を3本にしても桁が足りない。

ここは**動作点だけを厳しくする**。A=6 → A=4 にすると1トークンあたりに走る
expert が 8個中4個になり、捨てるぶんが 25% → 50% に増える。配分（どの層に
shared を何個置くか）が効く余地もそのぶん大きくなるはずで、効果量が伸びれば
同じ seed 3本でもベンチで見える。伸びなければ「動作点を倍にしても見えない」が
分かる。

**A 以外は report/07 と1つも変えない。** 校正・オラクル・探索・ルーター・分割・
評価セット・ベンチ・ブートストラップはすべて同じにする。変えるのは A と、
それに連動して決まるもの（一様の範囲 x=0..4、固定対照 uniform2）だけである。

1 seed は3段でできている。**済んだ段は飛ばす**。

  1. 探索  ``cmoe search`` 幅2 / 3 / 4  → result_logs/slimpajama_a4_w<W>_n16_seed<N>
  2. 採点  ``cmoe score``  一様 x=0..4  → result_logs/score_uniform_slimpajama_a4_n16_seed<N>
  3. 測定  ``cmoe run --bench`` 8構成   → result_logs/bench_slimpajama_a4_seed<N>

dense の基準は測らない。report/04 のものを取り込む（dense は変換していない
モデルなので、A にも校正にも seed にも配分にも依らない）。

  uv run python experiments/07_sparsity50/run.py --smoke   # 2層・8問
...
# A の半分＝ shared と routed が半々で、A=6 の uniform3 と同じ立ち位置である
...
def main(argv=None):
    ...
    if args.smoke:
        default_root = default_root / 'exp07_smoke'
    ...
    summarize(root / f'exp07_stages_seed{args.seed}.json', record)
