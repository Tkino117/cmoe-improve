"""16 の複数 seed をまとめる。GPU は使わない。

各 seed の ``bench_slimpajama_a2_seed<N>/summary.json`` と ``bench/*.json``、
それに ``score_uniform_slimpajama_a2_n16_seed<N>/score.json`` を読み、構成ごとに
...
06・07 との違いは動作点（N=8 / A=2）だけである。一様は x=0..2 の**3本**になり、
固定対照は A の半分の ``uniform1`` になる。読み方は 06・07 と同じ。
...
1. ``uniform1`` … 固定。選び方に評価指標が入らない
2. **校正で選んだ一様** … 一様3本を探索と同じ ``suffix_kl`` で採点した最良。
   探索も対照も「校正だけを見て選んだ1本」になるので、これが対等な比較である
3. 評価指標で最良の一様 … その指標を見てから3本の最良を取る。対照の側にだけ
...
def render(table, out):
    seeds = table['seeds']
    labels = table['labels']
    lines = [f'# 16 スパース率75%（N=8 / A={NACTIVE}）まとめ'
...
def main(argv=None):
    ...
    out = Path(args.out) if args.out else root / 'exp16_seeds'
