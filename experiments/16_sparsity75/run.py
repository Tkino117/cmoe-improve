...
[report/07](../../report/07_slimpajama-alloc-3seeds.md)（A=6、スパース率25%）と
[report/08](../../report/08_sparsity50-alloc-3seeds.md)（A=4、50%）は、探索配分が
**PPL では対照に勝ち、ベンチでは検出できない**という同じ形だった。50%にしても
ベンチの効果量は伸びきらなかった。

ここは**動作点だけをもう一段厳しくする**。A=4 → A=2 で、1トークンあたりに走る
expert が 8個中2個になり、捨てるぶんが 50% → 75% になる。ここまで削れば配分の
効き目は最大になるはずで、伸びなければ「どの動作点でも、この設計のベンチでは
見えない」が3点で言える。

**A 以外は report/07・08 と1つも変えない。** 校正・オラクル・探索・ルーター・
分割・評価セット・ベンチ・ブートストラップはすべて同じにする。変えるのは A と、
それに連動して決まるもの（一様の範囲 x=0..2、固定対照 uniform1）だけである。

**A=2 では一様が3本しかない。** x=0 は全層 routed、x=2 は全層 routing 無しで、
その間に x=1 が1本あるだけである。層ごとの候補も3個なので、探索が選べる幅は
25%・50%よりずっと狭い。**効果量が伸びない場合、それが「配分は効かない」なのか
「選べる余地が無い」なのかは、この実験だけでは分けられない**。report を書く
ときはそこを混ぜない。

1 seed は3段でできている。**済んだ段は飛ばす**。

  1. 探索  ``cmoe search`` 幅2 / 3 / 4  → result_logs/slimpajama_a2_w<W>_n16_seed<N>
  2. 採点  ``cmoe score``  一様 x=0..2  → result_logs/score_uniform_slimpajama_a2_n16_seed<N>
  3. 測定  ``cmoe run --bench`` 6構成   → result_logs/bench_slimpajama_a2_seed<N>

MMLU はここでは測らない。25%・50%と揃える必要があるので、3つのスパース率ぶんを
まとめて ``experiments/17_mmlu_bench`` が別に足す。

dense の基準は測らない。report/04 のものを取り込む（dense は変換していない
モデルなので、A にも校正にも seed にも配分にも依らない）。

  uv run python experiments/16_sparsity75/run.py --smoke   # 2層・8問
...
BENCH_BATCH = 32
# この実験の主題。N は動かさない — N を変えると expert 1個あたりのニューロン数
# （分割の粒度）まで動いて、比べているものが「配分」でなくなる
NEXPERTS = 8
NACTIVE = 2
# 動作点が名前に入る。A=6・A=4 の出力（report/07・08）と同じディレクトリに落ちる
# と、引数が食い違って ensure が止まる
TAG = f'a{NACTIVE}'
# 対照。先頭が cmoe run の対応のある比較の基準になるので固定対照を先に置く。
# A の半分＝ shared と routed が半々で、A=6 の uniform3・A=4 の uniform2 と同じ
# 立ち位置である。A=2 では一様が x=0/1/2 の3本しかない
BASELINE = f'uniform{NACTIVE // 2}'
UNIFORMS = [BASELINE] + [f'uniform{x}' for x in range(NACTIVE + 1)
...
def search_stage(seed, width, root, plan):
    """段1。層ごとの x を探す。"""
    out_dir = root / f'{CALIB}_{TAG}_w{width}_n{plan["nsamples"]}_seed{seed}'
...
def score_stage(seed, root, plan):
    ...
    out_dir = root / f'score_uniform_{CALIB}_{TAG}_n{plan["nsamples"]}_seed{seed}'
...
def main(argv=None):
    ...
    plan = {
        ...
        # 構成数がそのまま時間になる
        'uniforms': [BASELINE, f'uniform{NACTIVE}'] if args.smoke else UNIFORMS,
    ...
    if args.smoke:
        default_root = default_root / 'exp16_smoke'
    ...
    summarize(root / f'exp16_stages_seed{args.seed}.json', record)
